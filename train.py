...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    compute_loss = ComputeLoss(model, imgsz)  # init loss class
    ...
    # torch.save(ckpt, './pretrained.pt', _use_new_zipfile_serialization=False)
    # print("save pretrain weights")
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'mask', 'total', 'labels', 'img_size'))
        if RANK in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 7) % (
                    f'{epoch}/{epochs - 1}', mem, *mloss, targets[0].shape[0], imgs.shape[-1])
                pbar.set_description(s)

                # Plot
                if plots and ni < 3:
                    f = save_dir / f'train_batch{ni}.jpg'  # filename
                    Thread(target=plot_images, args=(imgs, targets[0], paths, f), daemon=True).start()
...
def parse_opt(known=False):
    ...
    parser.add_argument('--save_dir', type=str, default='.', help='save dir for runtime')
...
def main(opt):
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ckpt = opt.resume if isinstance(opt.resume, str) else get_latest_run(opt.save_dir)  # specified or most recent path
