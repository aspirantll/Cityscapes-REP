    ...
    for x in args:
        ...
        ### ops_nothing
            ...
            delta_ops = x.numel()
                           ...
                           'ROIAlign', 'PostProcessor', 'KeypointRCNNPredictor', 'Anchors', 'ELU',
                           'NaiveSyncBatchNorm', 'Upsample', 'Sequential', 'MemoryEfficientSwish']:
            pass
...
def compute_flops():
    ...
    image = torch.zeros(*input_size).cuda()
    _, ops, params = measure_model(model, image)
    print('ops: {:.2f}G\tparams: {:.2f}M'.format(ops / 2 ** 30, params / 2 ** 20))


if __name__ == "__main__":
    print("start to compute flops...")
    compute_flops()
