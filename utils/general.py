...
def xy2xyn(x, w=640, h=640):
    x[:, 0] = x[:, 0]/w  # top left x
    x[:, 1] = x[:, 1]/h  # top left y
    return x
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                if a[5] not in [3, 4, 5, 6]:
                    continue
                lt, rb = a[:2][::-1], a[2:4][::-1]
                lt, rb = convert_corner_to_corner(lt, rb, im0[i].shape[0], im0[i].shape[1], 1.5)
                cutout = im0[i][int(lt[0]):int(rb[0]), int(lt[1]):int(rb[1]), :]

                padw, padh = 0, 0
                wh = rb - lt
                if wh[0] > wh[1]:
                    padw = int(wh[0] - wh[1]) // 2
                else:
                    padh = int(wh[1] - wh[0]) // 2

                cutout = cv2.copyMakeBorder(cutout, padh, padh, padw, padw, cv2.BORDER_CONSTANT,
                                              value=(114, 114, 114))
            ...
            if len(ims):
                pred_cls2 = model(torch.Tensor(ims).cuda()).argmax(1)  # classifier prediction
                x[i][inds, 5] = pred_cls2.cpu().numpy()+3

                for k, cutout in enumerate(cutouts):
                    cv2.imshow(str(pred_cls2[k]), cutout)
                    cv2.waitKey()
...
def generate_coordinates(w=2048, h=1024):
    xm = torch.linspace(0, 2, w).view(
        1, 1, -1).expand(1, h, w)
...
def generate_corner(center, wh, max_h, max_w, delta_factor):
    ...
    return lt.astype(np.int32), rb.astype(np.int32)
...
def mask_to_polygons(mask):
    ...
    return [x.reshape(-1, 2) for x in res] if len(res)>0 else np.array(())


def max_polygon(polys):
    def max_rect_area(poly):
        wh = poly.max(0) - poly.min(0)
        return wh[0]*wh[1]

    max_poly = polys[0]
    max_area = max_rect_area(max_poly)
    for poly in polys:
        cur_area = max_rect_area(poly)
...
def process_mask(dets, seg_outs):
    seg = seg_outs
    _, seg_mask = seg.max(1)
    ...
    for i in range(b):
        ...
        for det in dets[i]:
            ...
            if det_np[5] in [7, 8]:
                lt = det_np[:2][::-1].astype(np.int32)
                rb = det_np[2:4][::-1].astype(np.int32)

                mask = seg_mask[i][lt[0]:rb[0]+1, lt[1]:rb[1]+1] == (det[5]-6)

                polygon = mask_to_polygons(mask.cpu().numpy())
                if len(polygon) != 0:
                    max_area, max_poly = max_polygon(polygon)
                    if max_area < 100:
                        continue

                    polygon = max_poly + lt[::-1]
        ...
        mask = seg_mask[i] == 3
        contours = mask_to_polygons(mask.cpu().numpy())
        for poly in contours:
            box = np.ones(6, dtype=np.float32)
            box[:2] = poly.min(0)
            box[2:4] = poly.max(0)
            box[5] = 9
        ...
        detections.append(np.vstack(boxes).reshape(-1, 6) if len(boxes) else np.empty((0, 6)))
        polygons.append(polys)

    return detections, polygons, seg_mask.cpu().numpy()
